...
struct snd_pxa2xx_i2sound_stream {
	...
	volatile int dma_running;	/* DMA running? */
  	...
  	snd_pcm_substream_t *asubs;	/* attached ALSA substream */

	snd_pcm_hardware_t ahw;		/* ALSA hw description */
};
...
struct snd_pxa2xx_i2sound_card {
	...
	snd_card_t *acard;		/* ALSA card */
	snd_pcm_t *apcm;		/* ALSA pcm */
...
static inline int pxa_25x_kind(void)
{
	...
	/* We can't use read_cpuid under L4 because we're at user level */ 
...
static snd_pcm_hw_constraint_list_t cnstr_period_sizes = {
        	.count = ARRAY_SIZE(period_sizes),
        	.list = period_sizes,
	};
static snd_pcm_hw_constraint_list_t cnstr_buffer_sizes = {
        	.count = ARRAY_SIZE(buffer_sizes),
        	.list = buffer_sizes,
	};

static snd_pcm_hw_constraint_list_t cnstr_rates;
					      ...
					      size_t buffer_size, size_t period_size)
{
	...
	if (!s->dma_dring_store) {
		printk("i2sound: couldn't allocate descriptor ring\n"); 
		s->dma_dring_size = 0;
		snd_pxa2xx_i2sound_ngn_free_buffer(s);
		return -ENOMEM;
	}
	...
	for (i = 0; i < periods; i++, p += period_size) {
		...
		for (j = 0; j < dma_dpp; j++, di++, d_buf += dma_xfer_size) {
			...
			if (s->id == SNDRV_PCM_STREAM_PLAYBACK) {
				...
				s->dma_dring[di].dtadr = SADR_PHYS;
				s->dma_dring[di].dcmd = DCMD_INCSRCADDR | DCMD_FLOWTRG |
							DCMD_BURST32 | DCMD_WIDTH4;
			}
			else {
				s->dma_dring[di].dsadr = SADR_PHYS;
...
static int snd_pxa2xx_i2sound_ngn_attach_asubs(snd_pcm_substream_t *asubs)
{
...
static void snd_pxa2xx_i2sound_ngn_detach_asubs(snd_pcm_substream_t *asubs)
{
	struct snd_pxa2xx_i2sound_stream *s = &card.engine.streams[asubs->stream];
...
static int snd_pxa2xx_i2sound_pcm_hw_params(snd_pcm_substream_t *asubs,
					    snd_pcm_hw_params_t * ahw_params)
{
...
static int snd_pxa2xx_i2sound_pcm_hw_free(snd_pcm_substream_t *asubs)
{
	snd_pxa2xx_i2sound_ngn_off((struct snd_pxa2xx_i2sound_stream*) asubs->private_data);
	return 0;
}

static int snd_pxa2xx_i2sound_pcm_prepare(snd_pcm_substream_t *asubs)
{
	return 0;
}

static int snd_pxa2xx_i2sound_pcm_trigger(snd_pcm_substream_t *asubs, int cmd)
{
...
static snd_pcm_uframes_t snd_pxa2xx_i2sound_pcm_pointer(snd_pcm_substream_t *asubs)
{
	struct snd_pxa2xx_i2sound_stream* s = 
		(struct snd_pxa2xx_i2sound_stream*) asubs->private_data;
...
static int snd_pxa2xx_i2sound_pcm_open(snd_pcm_substream_t *asubs)
{
	int ret;
...
static int snd_pxa2xx_i2sound_pcm_close(snd_pcm_substream_t *asubs)
{
...
static snd_pcm_ops_t snd_pxa2xx_i2sound_pcm_ops = {
...
static void snd_pxa2xx_i2sound_card_free_pcm(snd_pcm_t * apcm);
