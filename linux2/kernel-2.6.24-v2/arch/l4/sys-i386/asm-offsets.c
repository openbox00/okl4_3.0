...
main(void)
{
	...
	DEFINE(L4_SIGFRAME_FPU_OFFSET, offsetof (struct sigframe, fpstate._fxsr_env[0]));
	...
	DEFINE(L4_SIGRTFRAME_FPU_OFFSET, offsetof (struct rt_sigframe, fpstate._fxsr_env[0]));
	...
	DEFINE(CONFIG_IA32_VDSO_ENABLE_asm, 1);
#endif
	DEFINE(L4_RESTORE_FPU_PTR_OFFSET, offsetof (struct sigcontext, fpstate));
	DEFINE(L4_FPUSTATE_FPU_OFFSET, offsetof (struct _fpstate, _fxsr_env[0]));
	return 0; 
}
