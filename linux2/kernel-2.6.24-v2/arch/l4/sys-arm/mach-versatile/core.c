...
#define __io_address(n)		((void __iomem *)n)
...
void __init versatile_init(void)
{
...
#if defined(CONFIG_CELL)
	{
		extern unsigned long ETH_BASE;
		smc91x_resources[0].start = ETH_BASE;
		smc91x_resources[0].end = ETH_BASE + SZ_64K - 1;
	}
	platform_device_register(&smc91x_device);
#endif

#if 1	/* XXX need to fix up the virtual address :( -gl */
	{
		extern unsigned long SERIAL_BASE;
		uart0_device.res.start = SERIAL_BASE;
		uart0_device.res.end = SERIAL_BASE + SZ_4K - 1;
	}
#endif
	...
	{
		extern unsigned long CLCD_BASE;
		clcd_device.res.start = CLCD_BASE;
		clcd_device.res.end = CLCD_BASE + SZ_4K - 1;
	}
	{
		extern unsigned long KMI0_BASE;
		kmi0_device.res.start = KMI0_BASE;
		kmi0_device.res.end = KMI0_BASE + SZ_4K - 1;
	}
	{
		extern unsigned long KMI1_BASE;
		kmi1_device.res.start = KMI1_BASE;
		kmi1_device.res.end = KMI1_BASE + SZ_4K - 1;
	}
#endif
...
#if 1	/* make an initcall , needs to run AFTER amba_init()  -gl */
arch_initcall(versatile_init);
#endif
