 ...
 */
/*
 * OK Linux: Carl van Schaik
 *   This driver is modified to use TIMER1 not TIMER0 as the micro-kernel
 *   uses it.
 */
...
static irqreturn_t
pxa_ost1_interrupt(int irq, void *dev_id)
{
	int next_match;
	...
	if (c->mode == CLOCK_EVT_MODE_ONESHOT) {
		 ...
		 * OSMR1 and OSCR0 is still ahead of us, we've missed
		 ...
		 * writes to OSMR1 and the actual commit to the
		 ...
		 * target OSCR0 is "very close", to the OSMR1 load
		 * value, the update to OSMR1 might not get to the
		 * hardware in time and we'll miss that interrupt.
		 *
		 * To be safe, if the new OSMR1 is "very close" to the
...
static struct irqaction pxa_ost1_irq = {
	.name		= "ost1",
	.flags		= IRQF_DISABLED | IRQF_TIMER | IRQF_IRQPOLL,
	.handler	= pxa_ost1_interrupt,
	.dev_id		= &ckevt_pxa_osmr0,
};

//static void __init pxa_timer_init(void)  XXX do more arm like init -cvs
void __init pxa_timer_init(void)
{
	unsigned long clock_tick_rate;

	OIER &= ~(OIER_E1 | OIER_E2 | OIER_E3);
	OSSR = OSSR_M1 | OSSR_M2 | OSSR_M3;
	...
	setup_irq(IRQ_OST1, &pxa_ost1_irq);
...
static void pxa_timer_resume(void)
{
	 ...
	 * (OSMR1 - LATCH) is OSCR0 at the most recent system tick,
