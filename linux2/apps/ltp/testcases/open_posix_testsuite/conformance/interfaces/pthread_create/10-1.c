...
int main()
{
	...
	struct sigaction oact;
        ...
        memset(&inv_attr,0, sizeof(inv_attr));
	...
	act.sa_flags = 0;
        ...
        printf("INFO: calling sigaction(SIGSEGV, &act, NULL)\n");
	sigaction(SIGSEGV, &act, NULL);
	sigaction(SIGSEGV, &act, &oact);
        if(act.sa_handler != oact.sa_handler){
            printf("INFO: Installing the signal handler seems broken\n");
        } else {
            printf("INFO: The signal handler appears to be installed ok\n");
        }
	
	/* Create the thread with the invalid, uninitialized attributes object. */
        printf("INFO: calling pthread_create()\n");
	ret = pthread_create(&new_th, &inv_attr, a_thread_func, NULL);

        if(ret == EINVAL){
            printf("thread create returned EINVAL as per the posix spec for invalid attr\n");
            printf("PASS\n");
            return PTS_PASS;
        } else {
            printf("FAIL\npthread_create did not return EINVAL for invalid attr argument\n");
        }
	 ...
	 * the test fails. */
        printf("INFO: calling raise(SIGSEGV)\n");
	if(raise(SIGSEGV) != 0)
	{
