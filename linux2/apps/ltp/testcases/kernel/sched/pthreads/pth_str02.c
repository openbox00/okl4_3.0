	...
	if (test_limit || (num < num_threads)) {
		...
		if (pthread_attr_setdetachstate (&attr, PTHREAD_CREATE_JOINABLE))
			sys_error ("pthread_attr_setdetachstate failed", __LINE__);
                 ...
                 *   for pthread_create();
                 */
                fprintf(stderr, "before pthread_create - num: %d,  stacksize: %d\n", num, (int)stacksize);
		pcrterr = pthread_create (&th, &attr, thread, (void *)(num + 1));
                fprintf(stderr, "after pthread_create - num: %d, pcrterr: %d, test_limit: %d\n", num, pcrterr, test_limit);
		if (pcrterr != 0) {
			if (test_limit) {
			...
			if (pcrterr == EAGAIN) {
                ...
                fprintf(stderr, "before pthread_join - num: %d\n", num);
		pthread_join (th, (void *) NULL);
                fprintf(stderr, "after pthread_join - num: %d\n", num);
	}

        fprintf(stderr, "thread num=%d returning (0)\n", num);
        return(0);
