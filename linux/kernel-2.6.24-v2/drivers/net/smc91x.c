...
static int smc_drv_probe(struct platform_device *pdev)
{
	...
	int ret;
#ifdef SMC_USE_PXA_DMA
	struct resource *res_dma = platform_get_resource_byname(pdev, IORESOURCE_DMA, "smc91x-dma");
	if (!res_dma)
		res_dma = platform_get_resource(pdev, IORESOURCE_DMA, 2);
	if (!res_dma) {
		ret = -ENODEV;
		goto out;
	}
