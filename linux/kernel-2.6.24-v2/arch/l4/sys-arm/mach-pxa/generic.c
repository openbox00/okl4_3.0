...
//cvs #include <asm/mach/map.h>

#include <asm/arch/pxa-regs.h>
//cvs #include <asm/arch/gpio.h>
//cvs #include <asm/arch/udc.h>
//cvs #include <asm/arch/pxafb.h>
//cvs #include <asm/arch/mmc.h>
//cvs #include <asm/arch/irda.h>
//cvs #include <asm/arch/i2c.h>
//cvs #include <asm/arch/serial.h>
...
struct resource pxa_resource_ffuart[] = {
	{
		.start	= 0,
		.end	= 0 + 35,
		.flags	= IORESOURCE_MEM,
...
#if defined(CONFIG_CELL)
/* OK Linux driver stuff */

unsigned long PHYS_FFUART;
unsigned long PHYS_BTUART;
unsigned long PHYS_STUART;
unsigned long PHYS_HWUART;

void pxa_cell_init(void)
{
	/* L4-specific -gl,cvs */
	{
		word_t seg;
		...
		for (i = 0; i < segs->num_segments; i++) {
			if (segs->segments[i].segment == seg) {
				PHYS_FFUART = segs->segments[i].virt_addr;
printk("serial: %lx\n", PHYS_FFUART);
				break;
			}
		}
		...
		if (i == segs->num_segments)
			panic("Could not find SERIAL_BASE");
	}
	...
	// update ffuart
	pxa_resource_ffuart[0].start = PHYS_FFUART;
	pxa_resource_ffuart[0].end = PHYS_FFUART + 35;
}

int __init pxa_generic_init(void)
{
        pxa_cell_init();
	return 0;
}

arch_initcall(pxa_generic_init);
#endif
