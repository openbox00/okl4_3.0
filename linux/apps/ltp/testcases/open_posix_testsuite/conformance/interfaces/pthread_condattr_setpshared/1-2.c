...
int main()
{
	...
	for(i=0;i<NUM_OF_CONDATTR;i++)
	{
		...
		if(ret != 0)
		{
			printf("Test FAILED: Could not set pshared to PTHREAD_PROCESS_SHARED, error: %d, i: %d\n", ret, i);
			return PTS_FAIL;
		}
		...
		{
			printf("Test FAILED: obtaining the wrong process-shared attribute, expected PTHREAD_PROCESS_SHARED, but got: %d, i: %d\n", pshared, i);
			return PTS_FAIL;
		}
		...
		if((ret = pthread_condattr_destroy(&attr[i])) != 0)
		{
			fprintf(stderr, "Error at pthread_condattr_destroy() did not return 0, returned %d\n",ret);
			return PTS_UNRESOLVED;
		}
	}
