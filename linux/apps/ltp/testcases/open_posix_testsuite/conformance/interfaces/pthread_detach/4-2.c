...
int main()
{
	...
	if((ret = pthread_join(new_th, NULL)) == EDEADLK)
	{
		perror("Error joining thread\n");
		return PTS_UNRESOLVED;
	} else {
            printf("INFO: pthread_join returned %d\n", ret);
        }

	/* Detach the non-existant thread. */
        printf("INFO: Detaching non-existant thread, returned and pthread_joined"
                "should return ESRCH, no such process\n");
